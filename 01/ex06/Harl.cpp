...
// void	Harl::complain_filtered(const std::string level)
// {
// 	for (int i = 0; i < NUM_LEVEL; i++)
// 	{
// 		if (level == level_ary_[i])
...
int		Harl::convert_level_to_int(const std::string level)
{
	for (int i = 0; i < NUM_LEVEL; i++)
	{
		if (level == level_ary_[i])
...
void	Harl::complain_filtered(const std::string level)
{
	switch (convert_level_to_int(level))
	{
		case 0:
			debug();
			info();
